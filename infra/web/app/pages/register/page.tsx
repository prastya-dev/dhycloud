...
Register
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">Ulangi Kata sandi</label>
            <div className="relative">
              <input
                type={showPwd1 ? 'text' : 'password'} required
                value={password1} onChange={e => setPassword1(e.target.value)}
                placeholder="••••••••"
                className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm pr-20 focus:outline-none focus:border-gray-400 bg-white"
              />
              <button type="button" onClick={() => setShowPwd1(p => !p)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 hover:text-gray-600">
                {showPwd1 ? 'Sembunyikan' : 'Tampilkan'}
              </button>
            </div>
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-gray-900 text-white rounded-lg py-2.5 text-sm font-medium hover:bg-gray-700 disabled:opacity-50 transition-colors  transition-all">
            {/* {loading ? 'Memuat...' : 'Register'} */} Register
          </button>
        </form>
        ...
        <p className="text-center text-xs text-gray-400 mt-6">
          Sudah punya akun?{' '}
          <Link href="/pages/login" className="text-gray-600 underline">Login</Link>
        </p>
      </div>
    </main>
  )
